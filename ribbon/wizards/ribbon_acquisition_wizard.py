...
class RibbonMedalAcquisitionWizard(models.TransientModel):
    ...
    def getRibonImage(self):
        ...
        # acq_ribbons=self.env['ribbon.acquired.ribbon.wizard'].search([('partner_id','=',self.ribbon_holder.id)])
        acq_ribbons=self.acquired_ribbons
        acq_ribbons=acq_ribbons.sorted(key=lambda r: r.serial)
        for rec in acq_ribbons:
            print(rec.ribbon_id.name)
            if first_point==1:
            ...
            image_set=image_set+'<img style=" position:absolute;  left: '+str(position_left)+point_unit+';   width:'+str(point_length)+point_unit+';'\
                      + 'height:'+ str(point_height)+point_unit+';  border: 1px solid blue;z-index: 0;" src="../web/image/product.template/'+str(rec.ribbon_id.ribbon_tmpl_id.id)+'/image_1920"/>'+chr(10)
    ...
    def get_ribbons_for_order(self):
        data=[]
        for rec in self.acquired_ribbons:
            input = self.env["ribbon.order.ribbon.wizard"].create({
                'partner_id': self.ribbon_holder.id,
                'ribbon_id': rec.ribbon_id.id,
                'ribbon_product': 1,
                'extension': 1,
                'serial': rec.serial})
            data.append(input.id)
        self.order_ribbons = [(6, 0, data)]

    @api.onchange('order_ribbons')
    # TODO here to calculate ribbons price
    def get_ribbon_price(self):
        price=0
        for rec in self.order_ribbons :
            if rec.ribbon_product:
                price=price+10
...
class ribbonOrderRibbonWizard(models.TransientModel):
    _name="ribbon.order.ribbon.wizard"
    _description = "list of acquired rebbon"
    _order = "serial"
    partner_id = fields.Many2one("res.partner")
    ribbon_id = fields.Many2one("ribbon.regulation")
