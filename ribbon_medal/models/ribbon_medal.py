...
class RibbonMedalRibbonProduct(models.Model):
    _name="ribbon.medal.ribbon.product"
...
class ribbonAcquisitionRule(models.Model):
    _name="ribbon.medal.acquisition.rule"
...
class ribbonRegulation(models.Model):
    ...
    force_id=fields.Many2one('ribbon.medal.force',"Force Name")
    serial=fields.Integer("Serial")
    acquisition=fields.Many2one("ribbon.medal.acquisition.rule","Acquisition Rule")
...
class ribbonMedalForceUnit(models.Model):
    _name = "ribbon.medal.force.unit"
    _description = "Name of Units of a Force"
    name = fields.Char("Unit",translate=True)
    force_name=fields.Many2one("ribbon.medal.force","Force")
    address=fields.Char("address",translate=True)
    parent_unit=fields.Many2one("ribbon.medal.force.unit","Parent Unit",translate=True)
    chief_rank=fields.Many2one("ribbon.medal.rank","Rank of Chief",translate=True)
    chief=fields.Many2one("ribbon.medal.post","Chief",translate=True)

class RibbonMedalsPost(models.Model):
    _name="ribbon.medal.post"
