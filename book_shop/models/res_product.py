...
class ProductPublicCategory(models.Model):
    ...
    is_book_related=fields.Boolean("Book Related Category")
...
class productBook(models.Model):
    ...
    @api.onchange('writer_ids', 'publisher_id')
    def writer_id_changed(self):
        categories = []
        for categ in self.public_categ_ids:
            if categ.is_book_related==False:
                categories.append(categ.id)
        for writer in self.writer_ids:
            categories.append(writer.product_public_category_id.id)
        for publisher in self.publisher_id:
            categories.append(publisher.product_public_category_id.id)
        self.public_categ_ids = [(6, 0, categories)]
